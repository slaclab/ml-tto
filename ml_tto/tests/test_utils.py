...
from ml_tto.automatic_emittance.utils import calculate_bounding_box_coordinates, calculate_bounding_box_penalty, validate_beamsize_measurement_result
...
class TestUtils:
    def test_validate_beamsize_measurement_result(self):
        ...
        mock_result.centroids = np.stack([np.array([0, 0]), np.array([0, 0]), np.array([0, 0])]) + 1
        mock_result.metadata = MagicMock()
        mock_result.metadata.image_processor.roi = CircularROI(center=np.array([1, 1]), radius=2)

        validated_result, bb_penalties, log10_total_intensity = validate_beamsize_measurement_result(
            mock_result,
        ...
        assert np.allclose(log10_total_intensity, np.log10(mock_result.total_intensities))
