...
class HDF5Decoder:
    def __init__(self, verbose=True, use_attrdict=False, only_include=None):
    ...
    def mat2dict(self, hdf5):
        ...
        if self.only_include is not None:
            for var, found in self._found_include_var.items():
                if not found:
                    logging.warning(
                        f'Variable "{var}" was specified to be loaded'
    ...
    def unpack_mat(self, hdf5, depth=0, MATLAB_class=None, force=False):
        ...
        if "MATLAB_sparse" in hdf5.attrs:
            try:
                ...
                logging.error(
                    f"`scipy` not installed. To load the sparse matrix"
                    f" `{hdf5.name}`,"
                ...
                logging.error(
                    f"Tried loading the sparse matrix `{hdf5.name}`"
                    " with scipy, but"
        ...
        if isinstance(hdf5, (h5py._hl.group.Group)):
            ...
            for key in hdf5:
                ...
                unpacked = self.unpack_mat(
                    elem, depth=depth + 1, MATLAB_class=MATLAB_class
                )

                if (
                    MATLAB_class == "struct"
                    and len(elem) > 1
                    and isinstance(unpacked, dict)
                ):
    ...
    def convert_mat(self, dataset, depth, MATLAB_class=None):
        ...
        if not MATLAB_class and not self._has_refs(dataset):
            if self.verbose:
                message = "ERROR: not a MATLAB datatype: " + "{}, ({})".format(
        ...
        # if it is none of the above, we can convert to numpy array
            ...
            if self.verbose:
                message = "ERROR: MATLAB type not supported: " + "{}, ({})".format(
                    mtype, dataset.dtype
                )
                logging.error(message)
            return None
...
def loadmat(file, use_attrdict=False, only_include=None, verbose=True):
    ...
    """
    decoder = HDF5Decoder(
        verbose=verbose, use_attrdict=use_attrdict, only_include=only_include
    )
    ...
    if isinstance(file, str):
        ...
        if ext != ".mat":
            logging.warning(
                "Can only load MATLAB .mat file, this file type might "
    ...
    try:
        ...
        raise TypeError(
            f"{file} is not a MATLAB 7.3 file. Load with scipy.io.loadmat() instead."
        )
